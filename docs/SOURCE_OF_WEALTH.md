...
4. Operations or Compliance reviews evidence and uploaded documents, recording the SGD amount each item supports for every linked event. Placeholder sample documents do not count toward verified coverage. Historical sources need a relevance rationale after the configured age threshold.
...
Compliance can change **Review thresholds** in the wealth register. Defaults are demonstration parameters: 80% standard coverage, 100% enhanced coverage, 10% materiality, 10% reconciliation tolerance, 365-day evidence relevance prompt. These are not regulatory standards or a probability of legitimacy. Old transactions can be supported by historical evidence with documented relevance. Coverage uses explicit reviewed SGD amounts, taking the strongest usable independent source for each event rather than summing potentially overlapping sources. The evidence matrix shows claimed, supported and uncovered amounts with category-specific guidance. Changes to linked events invalidate evidence reviews; older reviews without recorded amounts need re-review. Material sources require full corroboration. Narrative citations must support claims in the body, not merely appear in the evidence register.
...
## Connected review experience
Evidence citations in the write-up open the relevant source card. **Readable report** downloads an escaped, printable HTML assessment; the JSON review pack retains full historical snapshots. Decision snapshots include policy, client risk and document metadata.

**Request evidence** creates a tracked case follow-up with an owner, instructions, priority and due date. Client-owned requests appear in the client portal; clients respond, and staff accept or reopen them. Follow-ups do not override document or approval gates.

**Work queue** combines these follow-ups with live lifecycle actions, overdue filters and client responses. The dashboard provides a role-specific shortcut, and the header search finds relationships by client, case ID, owner or country.
